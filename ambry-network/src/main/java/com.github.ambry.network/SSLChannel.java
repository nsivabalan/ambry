...
public class SSLChannel implements ReadableByteChannel, WritableByteChannel {
  ...
  public SSLChannel(SSLFactory sslFactory, String connectionId, SocketChannel socketChannel, SelectionKey key,
      ...
      switch (handshakeStatus) {
        case NEED_TASK:
          log.trace(
              ...
              connectionId, appReadBuffer.position(), netReadBuffer.position(), netWriteBuffer.position());
        ...
        case NEED_WRAP:
          ...
          log.trace(
              ...
              connectionId, handshakeResult, appReadBuffer.position(), netReadBuffer.position(),
  ...
  private SSLEngineResult handshakeWrap(Boolean doWrite)
      throws IOException {
    log.trace("SSLHandshake handshakeWrap", connectionId);
  ...
  private SSLEngineResult handshakeUnwrap(Boolean doRead)
      throws IOException {
    log.trace("SSLHandshake handshakeUnwrap", connectionId);
    ...
    if (dst.remaining() > 0) {
      ...
      do {
        ...
        if (unwrapResult.getHandshakeStatus() != SSLEngineResult.HandshakeStatus.NOT_HANDSHAKING) {
          log.trace(
              "SSLChannel Read begin renegotiation connectionId {}, appReadBuffer pos {}, netReadBuffer pos {}, netWriteBuffer pos {}",
              connectionId, appReadBuffer.position(), netReadBuffer.position(), netWriteBuffer.position());
          handshake();
          break;
        }
